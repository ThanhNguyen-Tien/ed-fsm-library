...
#define M_EVENT_DEF(name)\
	...
	extern void name##EventHandler_(void *data);
...
#define _M_EVENT_POST_2(name, data)\
	Event_Post(name.index, &data)

// Empty Event
#define _M_EVENT_POST_1(name)\
	Event_Post(name.index, NULL)
...
#define _M_EVENT_INIT_1(name)\
	Event_Init(&name, 0, &name##Handler_)

#define _M_EVENT_INIT_2(name, size)\
	Event_Init(&name, size, &name##Handler_)
